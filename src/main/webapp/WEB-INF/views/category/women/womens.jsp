...
<body>
	...
	document.getElementById("load-more-button").addEventListener("click", function () {
	    ...
	    // 이미지 경로를 저장하는 배열 확장
	    var imagePaths = [
	        "${request.getContextPath()}/resources/img/women/awomen1.png",
	        "${request.getContextPath()}/resources/img/women/awomen2.png",
	        "${request.getContextPath()}/resources/img/women/awomen3.png",
	        "${request.getContextPath()}/resources/img/women/awomen4.png",
	    ...
	    for (var i = 0; i < itemsToAdd; i++) {
	        ...
	        imageItem.innerHTML =`
	            <div class="card">
	                <img src="${imagePaths[totalImageItems + i]}"
	                    class="card-img-top" alt="women Image ${totalImageItems + i + 1}">
	                <div class="card-body">
	                    <p class="card-title mb-0" style="font-weight: bold;">For Women ${totalImageItems + i + 1}</p>
	                    <p class="card-text mb-1">Women Item ${totalImageItems + i + 1}</p>
	                    <p class="card-text mt-3" style="font-weight: bold;">1,000,000원</p>
	                    <a href="<%=request.getContextPath()%>/product/details/${totalImageItems + i + 1}" class="stretched-link"></a>
	                </div>
	            </div>
	          `;
	    ...
	    // 이미지 항목을 추가한 후에는 totalImageItems를 업데이트해야 합니다.
	    totalImageItems += itemsToAdd;

	    // 모든 항목을 로드했을 때 "더보기" 버튼을 숨김 (예: 이미지 항목이 더 이상 없을 경우)
	    if (totalImageItems >= imagePaths.length) {
